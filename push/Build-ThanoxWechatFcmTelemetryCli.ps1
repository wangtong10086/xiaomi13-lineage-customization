...
param(
    ...
    [string]$OutputPath = (Join-Path $PSScriptRoot 'work\thanox-wechat-fcm-telemetry-cli.jar')
)
...
$source = Join-Path $PSScriptRoot 'thanox-helper\ThanoxWechatFcmTelemetryCli.java'
$outputFull = [IO.Path]::GetFullPath($OutputPath)
$outputDir = Split-Path -Parent $outputFull
$stage = Join-Path ([IO.Path]::GetTempPath()) ("thanox-wechat-fcm-telemetry-" + [guid]::NewGuid().ToString('N'))
